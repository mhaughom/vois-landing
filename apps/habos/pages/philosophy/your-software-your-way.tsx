...
const practiceItemColors = ['bg-blue-500', 'bg-amber-500', 'bg-emerald-500', 'bg-violet-500'];

const YourSoftwareYourWay: React.FC = () => {
  const { t } = useTranslation('philosophy-your-software-your-way');
  const practiceItems = t('practiceItems', { returnObjects: true }) as Array<{ name: string; desc: string }>;
      ...
      <main className="pt-32 pb-24 px-6 md:px-12">
        <div className="max-w-3xl mx-auto">
          ...
          >
            ...
            <h2 className="text-2xl font-serif text-slate-900 mt-12 mb-4">{t('section1.heading')}</h2>

            <p>{t('section1.body')}</p>

            {/* What flexibility looks like */}
            <div className="not-prose my-12 bg-slate-50 rounded-2xl border border-slate-100 p-6">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-4">{t('practiceLabel')}</p>
              <div className="space-y-3">
                {practiceItems.map((item, i) => (
                  <div key={item.name} className="flex items-baseline gap-3">
                    <span className={`w-2 h-2 rounded-full ${practiceItemColors[i]} flex-shrink-0 mt-1.5`} />
                    <p className="text-sm text-slate-700"><strong className="text-slate-900">{item.name}</strong> — {item.desc}</p>
                  </div>
                ))}
              </div>
            </div>

            <h2 className="text-2xl font-serif text-slate-900 mt-12 mb-4">{t('section2.heading')}</h2>
