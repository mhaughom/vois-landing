...
const ProductBusinesses: React.FC = () => {
  const { t } = useTranslation('solutions-product-businesses');
  const stats = t('body.section4.stats', { returnObjects: true }) as string[][];
      ...
      <main className="pt-32 pb-24 px-6 md:px-12">
        <div className="max-w-3xl mx-auto">
          ...
          <motion.div initial="hidden" animate="visible" variants={fade} transition={{ duration: 0.6 }}>
            <p className="text-sm font-semibold text-emerald-600 tracking-widest uppercase mb-4">{t('hero.category')}</p>
          ...
          >
            ...
            <div className="not-prose my-8">
              <div className="bg-emerald-50 rounded-2xl p-6 border border-emerald-100">
                <p className="text-base text-slate-800 font-medium italic leading-relaxed mb-3">
                  {t('body.section3.voiceNote')}
                </p>
                <p className="text-sm text-emerald-700">
                  {t('body.section3.voiceResult')}
                </p>
              </div>
            </div>
            ...
            <h2 className="text-2xl font-serif text-slate-900">{t('body.section4.heading')}</h2>

            <div className="not-prose my-10">
              <div className="bg-slate-50 rounded-2xl border border-slate-100 overflow-hidden">
                <div className="grid grid-cols-2 divide-x divide-slate-100">
                  <div className="p-6 text-center">
                    <p className="text-sm text-slate-400 uppercase tracking-wider mb-2">{t('body.section4.before.label')}</p>
                    <p className="text-3xl font-bold text-slate-300">{t('body.section4.before.price')}</p>
                    <p className="text-sm text-slate-400 mt-1">{t('body.section4.before.note')}</p>
                  </div>
                  <div className="p-6 text-center">
                    <p className="text-sm text-emerald-600 uppercase tracking-wider mb-2">{t('body.section4.after.label')}</p>
                    <p className="text-3xl font-bold text-slate-900">{t('body.section4.after.price')}</p>
                    <p className="text-sm text-slate-500 mt-1">{t('body.section4.after.note')}</p>
                  </div>
                </div>
