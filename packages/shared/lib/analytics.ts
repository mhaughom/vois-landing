...
// The consuming app must call initAnalytics() with its consent manager's isPostHogReady
let _isPostHogReady: () => boolean = () => false;

export function initAnalytics(isPostHogReady: () => boolean) {
  _isPostHogReady = isPostHogReady;
}
...
export const Analytics = {
  ...
  getDistinctId: (): string | null => {
    if (!_isPostHogReady()) return null;
