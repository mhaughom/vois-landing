...
interface NavbarProps {
  ...
  /** Optional custom logo component (e.g. AnimatedHabosIcon). Falls back to <img> if not provided. */
  LogoComponent?: React.FC<{ isHovered: boolean; className: string }>;
}
...
export const Navbar: React.FC<NavbarProps> = ({ variant = 'habos', onOpenWaitlist, onResetDemo, isDemoActive, LogoComponent }) => {
      ...
      >
        ...
        {isDemoActive && onResetDemo ? (
          ...
          >
            ...
            >
              {LogoComponent ? (
                <LogoComponent isHovered={logoPillHovered} className="h-8 w-8 sm:h-12 sm:w-12 -ml-1" />
              ) : (
